...
library(Seurat)
library(anndata)
use_python("C:/Users/test/anaconda3/python.exe")
...
```{r}
# Extrayez les composantes principales
V <- Embeddings(seurat_object, reduction = "pca")

# Appliquez la normalisation cosinus
V_cos <- cosine_normalize(V, 1)

# Extrayez les métadonnées
...
```{r}
seurat_object <- RunHarmony(seurat_object, group.by.vars = "Id")
```
...
```{r}
DimPlot(seurat_object, reduction = "umap", group.by = "Id", shuffle = TRUE)
...
```{r}
dir.create("G:/Data processing pipeline 0.1 Yohan/scRNA/Output_iterative/adata_postharmony", recursive = TRUE)

write_h5ad(adata, filename = "G:/Data processing pipeline 0.1 Yohan/scRNA/Output_iterative/adata_postharmony/adata_postharmony.h5ad")
```
